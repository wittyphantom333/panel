...
class MakeNodeCommand extends Command
{
    protected $signature = 'p:node:make
                            ...
                            {--daemonListeningPort= : Enter the buddy listening port.}
                            {--daemonSFTPPort= : Enter the buddy SFTP listening port.}
    ...
    public function handle()
    {
        ...
        $data['daemonListen'] = $this->option('daemonListeningPort') ?? $this->ask('Enter the buddy listening port', '8080');
        $data['daemonSFTP'] = $this->option('daemonSFTPPort') ?? $this->ask('Enter the buddy SFTP listening port', '2022');
        $data['daemonBase'] = $this->option('daemonBase') ?? $this->ask('Enter the base folder', '/var/lib/pteranodon/volumes');
